...
package("libcurl")
    ...
    add_configs("apple_sectrust", {description = "Use Apple SecTrust native certificate verification.", default = false, type = "boolean" })
    ...
    on_load(function (package)
        ...
        if package:is_plat("macosx", "iphoneos") then
            ...
            if package:config("apple_sectrust") then
    ...
    on_install("windows", "mingw", "linux", "bsd", "macosx", "iphoneos", "cross", "android", "wasm", function (package)
        ...
        if package:is_plat("macosx") then
            table.insert(
                configs,
                "-DUSE_APPLE_SECTRUST=" ..
                (package:config("apple_sectrust") and "ON" or "OFF")
            )

            if package:config("apple_sectrust") then
                -- Do not bake a build-machine CA bundle/path into curl.
                table.insert(configs, "-DCURL_CA_BUNDLE=none")
                table.insert(configs, "-DCURL_CA_PATH=none")
            end
        end
